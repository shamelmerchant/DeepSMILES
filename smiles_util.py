...
def conv_smile_to_vec(smiles,max_len = 100, append = True):
    ...
    for smile in smiles:
        ...
        if len(smile) < max_len:
            ...
            vec = [token2idx[char] for char in smile]
            smile_vec.append(vec)
            #for j,char in enumerate(smile):
            #    smile_vec[i,j] = token2idx[char]
        else:
            smile_not_processed.append(smile)

    return np.array(smile_vec), tokens, token2idx, idx2token, smile_not_processed
